...
| **🪪 Report Cards** | Toggle **🎓 By Student** (all classes for one student) or **📋 By Class** (walk the class roster student-by-student, one class at a time). Optional **📝 Quiz only** hides attendance, homework, and comments so the card is just quiz averages + per-quiz scores. Each quiz row includes **Class avg** (roster mean %). Non-SAT sections also show an overall **Class average** tile. **SAT** classes pair **Math + ELA** (same practice date/title) into a **Combined total** (/1600) shown **above** the per-subject sections — including cross-session (e.g. morning Math + afternoon ELA) in one table. **Quiz scores are editable inline** (same records as 📝 Grades — type a score and tab/click away; clear the box to remove). Full card otherwise: attendance, homework, quizzes, **teacher comment**. **🖨 Print** or **Export CSV**. |
...
- **Quiz sheet template** (English; pre-filled rosters from live Default plan): [`templates/PremierPlus_Quiz_Sheet_Template.xlsx`](./templates/PremierPlus_Quiz_Sheet_Template.xlsx) — one score tab per class; **Import** tab auto-maps all scores via formulas. Roster dump: [`templates/PremierPlus_Class_Rosters.csv`](./templates/PremierPlus_Class_Rosters.csv). Upload xlsx to Google Drive → open with Google Sheets.
