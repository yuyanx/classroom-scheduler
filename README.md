...
| **🪪 Report Cards** | Toggle **🎓 By Student** (all classes for one student) or **📋 By Class** (walk the class roster student-by-student, one class at a time). Optional **📝 Quiz only** hides attendance, homework, and comments so the card is just quiz averages + per-quiz scores. Non-SAT class sections include the student’s **Quiz average** plus the **Class average** (and per-quiz **Class %**). **SAT** classes show student scores only (no class average) and pair **Math + ELA** (same practice date/title) into a **Combined total** (/1600) — including cross-session (e.g. morning Math + afternoon ELA) in one table. Full card otherwise: attendance, homework, quizzes, **teacher comment**. **🖨 Print** or **Export CSV**. |
